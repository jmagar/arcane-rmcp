...
- `src/cli/watch.rs` — `rustcane watch` subcommand for live file-system monitoring.
- `plugins/rustcane/monitors/` — plugin monitor definitions for event-driven automation.
- `plugins/rustcane/gemini-extension.json` — Gemini extension manifest for multi-platform plugin distribution.
...
- `plugins/README.md`, `plugins/rustcane/README.md`, `plugins/rustcane/CLAUDE.md` — plugin-level documentation and agent guidance.
...
- `plugins/rustcane/hooks/plugin-setup.sh` — significant simplification; reduced from ~500 to ~50 lines by extracting reusable logic and removing duplication.
...
- Action-based dispatch: single `rustcane` MCP tool with `action` parameter routing
- Both transports: Streamable HTTP (`rustcane serve`) and stdio (`rustcane mcp`)
- Bearer token authentication via `RUSTCANE_MCP_TOKEN`
- Google OAuth authentication via `RUSTCANE_MCP_AUTH_MODE=oauth` (issues RS256 JWTs)
...
- MCP resources: exposes tool schema at `rustcane://schema/mcp-tool`
...
- `default_data_dir()` — detects container vs bare-metal, returns `/data` or `~/.rustcane`
...
- `config.rustcane.toml` — fully annotated config template
- `.env.rustcane` — documented secrets template
