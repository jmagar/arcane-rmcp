//! REST API handlers — `POST /v1/rustcane`, `GET /health`, `GET /status`, `GET /openapi.json`.
//!
...
use crate::actions::{execute_service_action, required_scope_for_action, ArcaneAction};
use crate::server::{AppState, AuthPolicy};
use crate::token_limit::MAX_RESPONSE_BYTES;

/// Request body for `POST /v1/rustcane`.
///
...
/// typed `ArcaneAction` before calling `ArcaneService`.
#[derive(Deserialize)]
...
/// `POST /v1/rustcane` — dispatches an action by name.
///
