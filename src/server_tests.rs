...
fn non_loopback_no_auth_without_gateway_is_rejected() {
    ...
    assert!(error.to_string().contains("RARCANE_MCP_NO_AUTH=true"));
}
...
fn invalid_public_url_is_rejected() {
    ...
    assert!(error
        .to_string()
        .contains("RARCANE_MCP_PUBLIC_URL is invalid"));
}

#[test]
fn wildcard_public_url_is_rejected() {
    let mut config = config("0.0.0.0");
    config.mcp.auth.public_url = Some("https://*.rarcane.com".into());
    let error = resolve_auth_policy_kind(&config, true).unwrap_err();
    assert!(error
        .to_string()
        .contains("RARCANE_MCP_PUBLIC_URL must not contain wildcard hosts"));
}
