...
def validate_schema() -> None:
    ...
    require(schema.get("properties", {}).get("kind", {}).get("const") == "rarcane_scaffold_intent", f"{SCHEMA}: kind const drifted")
...
def validate_payload(payload: object, source: Path) -> None:
    ...
    require(payload["kind"] == "rarcane_scaffold_intent", f"{source}: invalid kind")
...
def main() -> int:
    try:
        ...
        for rarcane in examples:
            validate_payload(load_json(rarcane), rarcane)
