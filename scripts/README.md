...
Pre-commit guard that rejects staged `.env`, `.env.local`, `.env.prod`, etc. `.env.rarcane` is allowed.
...
```bash
RARCANE_MCP_TOKEN=... bash scripts/generate-cli.sh
just generate-cli
```
...
Stops, rebuilds, and restarts the `rarcane-mcp` service. Detects the active service manager automatically: prefers a systemd user unit (`rarcane-mcp.service`), falls back to Docker Compose. Useful after an in-place binary update without a full `docker compose build`.
...
```bash
RARCANE_MCP_TOKEN=... scripts/test-mcp-auth.sh
...
PLUGIN_ROOT=plugins/rarcane scripts/validate-plugin-layout.sh
