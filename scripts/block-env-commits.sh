...
#   file (other than .env.rustcane). This prevents accidentally committing API
#   keys, OAuth secrets, or bearer tokens.
#
# What it allows:
#   - .env.rustcane  (the committed template — no real secrets)
#
# What it blocks:
...
# Find any .env* files in the staged list, excluding .env.rustcane.
# Regex breakdown:
...
if [[ -n "$blocked" ]]; then
    ...
    echo "" >&2
    echo "Only .env.rustcane is allowed to be committed." >&2
