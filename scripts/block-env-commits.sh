...
#   file (other than .env.rarcane). This prevents accidentally committing API
#   keys, OAuth secrets, or bearer tokens.
#
# What it allows:
#   - .env.rarcane  (the committed template — no real secrets)
#
# What it blocks:
...
# Find any .env* files in the staged list, excluding .env.rarcane.
# Regex breakdown:
...
if [[ -n "$blocked" ]]; then
    ...
    echo "" >&2
    echo "Only .env.rarcane is allowed to be committed." >&2
