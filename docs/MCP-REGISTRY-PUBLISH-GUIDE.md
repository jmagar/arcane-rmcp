...
- You own the domain used in the `name` field (e.g. `tv.tootie` in `tv.tootie/rustcane-mcp`)
...
e.g. `github.com/jmagar/rustcane-mcp`.
...
If your `name` is `tv.tootie/rustcane-mcp`, you must authenticate with DNS for
`tv.tootie`. If your `name` is `github.com/jmagar/rustcane-mcp`, use GitHub OAuth.
