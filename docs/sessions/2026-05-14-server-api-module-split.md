...
Refactor the rustcane Rust MCP server: create a CLAUDE.md for `apps/web/` guiding Aurora design system usage, then fix improper module organization (`src/mcp/` containing REST API code), enforce the no-`mod.rs` rule via a git hook, update `docs/PATTERNS.md` to match, add `deny.toml`, and push everything.
...
| Public API | `rustcane::mcp::{AppState, AuthPolicy, router}` | `rustcane::server::{AppState, AuthPolicy, router}` |
...
- **Breaking public API**: `rustcane::mcp::{AppState, AuthPolicy}` is now `rustcane::server::*`. Any downstream crate importing from the library will break. Rollback: revert `src/lib.rs` module declarations and restore old `src/mcp.rs`.
