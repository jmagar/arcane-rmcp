...
- Artifact upload name had `-linux-` hardcoded for all targets: Windows artifact would have been named `rustcane-linux-windows-x86_64`
...
# Verify output
stat target/x86_64-pc-windows-gnu/release/rustcane.exe
file target/x86_64-pc-windows-gnu/release/rustcane.exe
...
| Windows binary | Not produced | `rustcane-windows-amd64.exe` in `bin/` and as release asset |
| Artifact upload action version | `@v7` (nonexistent) | `@v4` |
| Platform isolation | `fail-fast: true` (default) | `fail-fast: false` |
| Artifact names | `rustcane-linux-x86_64`, `rustcane-linux-windows-x86_64` | `rustcane-x86_64`, `rustcane-windows-x86_64` |
...
| `file target/x86_64-pc-windows-gnu/release/rustcane.exe` | PE32+ x86-64 exe | `PE32+ executable for MS Windows 5.02 (console), x86-64` | PASS |
| `stat ...rustcane.exe` (size) | Non-zero binary | 33,764,667 bytes | PASS |
