...
worktree: /home/jmagar/workspace/rarcane  2a4599c [full-review-remediation]
---
...
- Treated `rarcane:write` as satisfying read checks instead of keeping the obsolete `rarcane:admin` wording.
...
- Updated the schema-doc checker to read `ACTION_SPECS` rather than regexing deleted `RARCANE_ACTIONS` and `READ_ONLY_ACTIONS` constants.
...
- `README.md`, `AGENTS.md`, `CLAUDE.md`, `docs/AUTH.md`, `docs/MCP_SCHEMA.md`, `docs/PATTERNS.md`, `plugins/README.md`, `scripts/README.md`, `scripts/check-schema-docs.py`, `config.rarcane.toml`, `CHANGELOG.md`: stale contract and workflow documentation updates.
...
| Release assets | Raw binary artifacts did not match installer tarball names | Release workflow creates `rarcane-linux-x86_64.tar.gz` and `rarcane-linux-aarch64.tar.gz` tarballs matching installer platform naming |
...
- Did not keep `rarcane:admin`; docs and code now use `rarcane:write` as the elevated scope.
