...
worktree: /home/jmagar/workspace/rustcane  2a4599c [full-review-remediation]
---
...
- Treated `rustcane:write` as satisfying read checks instead of keeping the obsolete `rustcane:admin` wording.
...
- Updated the schema-doc checker to read `ACTION_SPECS` rather than regexing deleted `RUSTCANE_ACTIONS` and `READ_ONLY_ACTIONS` constants.
...
- `README.md`, `AGENTS.md`, `CLAUDE.md`, `docs/AUTH.md`, `docs/MCP_SCHEMA.md`, `docs/PATTERNS.md`, `plugins/README.md`, `scripts/README.md`, `scripts/check-schema-docs.py`, `config.rustcane.toml`, `CHANGELOG.md`: stale contract and workflow documentation updates.
...
| `/status` | Returned local metadata without calling the service | Calls `ArcaneService::status()` and merges redacted local metadata |
| CLI echo/watch | Missing echo message and invalid interval could silently default | Invalid inputs now fail parsing |
| Release assets | Raw binary artifacts did not match installer tarball names | Release workflow creates `rustcane-linux-x86_64.tar.gz` and `rustcane-linux-aarch64.tar.gz` tarballs matching installer platform naming |
...
- Did not keep `rustcane:admin`; docs and code now use `rustcane:write` as the elevated scope.
