...
| `src/config.rs:263` | Modified | Reject invalid `RARCANE_MCP_AUTH_MODE` values instead of silent Bearer coercion |
...
| `plugins/rarcane/gemini-extension.json` | Modified | `"secret"` → `"sensitive"` |
...
| `plugins/rarcane/README.md:30` | Modified | Clarify Gemini uses inline `mcpServers`, not `.mcp.json` |
| `plugins/rarcane/hooks/plugin-setup.sh:55` | Modified | Map `CLAUDE_PLUGIN_OPTION_NO_AUTH` → `RARCANE_MCP_NO_AUTH` |
...
ssh squirts "for f in axon apprise gotify lab rmcp-rarcane syslog tailscale unifi unraid; do
...
| `RARCANE_MCP_AUTH_MODE=typo` | Silently used Bearer | Hard error at startup |
| `docker run ... bash` | Ran `rarcane bash` (unknown subcommand error) | Execs `bash` directly under gosu |
...
- [PR #1](https://github.com/jmagar/rarcane/pull/1) — merged
