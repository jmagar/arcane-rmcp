...
| `src/config.rs:263` | Modified | Reject invalid `RUSTCANE_MCP_AUTH_MODE` values instead of silent Bearer coercion |
...
| `plugins/rustcane/gemini-extension.json` | Modified | `"secret"` → `"sensitive"` |
...
| `plugins/rustcane/README.md:30` | Modified | Clarify Gemini uses inline `mcpServers`, not `.mcp.json` |
| `plugins/rustcane/hooks/plugin-setup.sh:55` | Modified | Map `CLAUDE_PLUGIN_OPTION_NO_AUTH` → `RUSTCANE_MCP_NO_AUTH` |
...
ssh squirts "for f in axon apprise gotify lab rmcp-rustcane syslog tailscale unifi unraid; do
...
| `RUSTCANE_MCP_AUTH_MODE=typo` | Silently used Bearer | Hard error at startup |
| `docker run ... bash` | Ran `rustcane bash` (unknown subcommand error) | Execs `bash` directly under gosu |
...
- [PR #1](https://github.com/jmagar/rustcane/pull/1) — merged
