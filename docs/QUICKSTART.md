...
```bash
RUSTCANE_MCP_TOKEN=a3f2c1... cargo run -- serve
```
...
```json
{
  "mcpServers": {
    "rustcane": {
      "command": "/path/to/rustcane/target/debug/rustcane",
      "args": ["mcp"],
      "env": { "RUST_LOG": "warn" }
    }
  }
...
- For OAuth setup, set `RUSTCANE_MCP_AUTH_MODE=oauth` and the `RUSTCANE_MCP_GOOGLE_*` env vars — see the env var table in the README.

## Checklist for adapting this template

Use this when creating a real service from rustcane:
