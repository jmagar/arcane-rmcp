...
`apps/web/lib/template.ts` defines the service display name, endpoints, and optional API base URL. `NEXT_PUBLIC_RUSTCANE_API_BASE_URL` should be empty by default so the UI uses same-origin API calls when served by the Rust binary.

Use `apps/web/.env.rustcane` for local web development overrides only.
