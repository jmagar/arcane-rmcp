...
1. Replace every `rustcane` / `Arcane` / `RUSTCANE_` identifier with your service name
2. Update `userConfig` / `settings` in all three manifests to match your service's credentials
3. Update `skills/rustcane/SKILL.md` — action table, parameters, response shapes, workflows
4. Set `brandColor` and `defaultPrompt` in `.codex-plugin/plugin.json`
5. Update `hooks/plugin-setup.sh` env var block to match your service's `RUSTCANE_*` vars
