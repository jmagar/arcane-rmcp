...
- Ensures `rustcane` is available on `PATH`
- Calls `rustcane setup plugin-hook "$@"`
...
### `skills/rustcane/SKILL.md`

Three-tier structured documentation for the `rustcane` MCP tool, used by both Claude Code and Codex to understand when and how to invoke the tool.

**Tier 1** (above the fold): tool name, quick action table, most common usage.  
**Tier 2**: full action reference — parameters, types, rustcane calls, response shapes.  
...
1. Replace all `rustcane` / `Arcane` / `RUSTCANE_` identifiers with your service name.
2. Update `userConfig` in both `plugin.json` files to match your service's credential fields.
3. Update `skills/rustcane/SKILL.md` with your actual actions, parameters, and examples.
