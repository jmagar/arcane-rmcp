# rustcane — Claude Code instructions

## What this project is

A reusable Rust template for building MCP servers with the rmcp crate. The binary is named `rustcane`. All stub identifiers (`Arcane*`, `RUSTCANE_*`) are renamed when the template is used for a real service.

## Module map

| File | Role |
|------|------|
| `src/rustcane.rs` | `ArcaneClient` — HTTP/API transport stub; one method per remote operation |
...
| `src/api.rs` | REST API handlers: `POST /v1/rustcane`, `GET /health`, `GET /status` |
...
1. **`src/rustcane.rs`** — add `pub async fn your_action(&self, ...) -> Result<Value>` with the actual HTTP/API call (or stub).
...
| `AuthPolicy::TrustedGatewayUnscoped` | `RUSTCANE_NOAUTH=true` on non-loopback behind an authz-enforcing gateway | No auth middleware; scope checks bypassed |
...
Auth is selected in `build_auth_policy()` in `main.rs`. Scopes are `rustcane:read` and `rustcane:write` (write satisfies read). `help` requires no scope. Unknown actions get `DENY_SCOPE`.
...
| `RUSTCANE_MCP_HOST` | `127.0.0.1` | Bind host |
| `RUSTCANE_MCP_PORT` | `40060` | Bind port |
| `RUSTCANE_MCP_NO_AUTH` | `false` | Disable auth (loopback only) |
| `RUSTCANE_MCP_TOKEN` | — | Static bearer token |
| `RUSTCANE_MCP_ALLOWED_HOSTS` | — | Extra comma-separated Host header values |
| `RUSTCANE_MCP_ALLOWED_ORIGINS` | — | Extra comma-separated CORS origins |
| `RUSTCANE_MCP_PUBLIC_URL` | — | Public URL for OAuth metadata endpoints |
| `RUSTCANE_MCP_AUTH_MODE` | `bearer` | `bearer` or `oauth` |
| `RUSTCANE_MCP_GOOGLE_CLIENT_ID` | — | Google OAuth client ID |
| `RUSTCANE_MCP_GOOGLE_CLIENT_SECRET` | — | Google OAuth client secret |
| `RUSTCANE_MCP_AUTH_ADMIN_EMAIL` | — | OAuth admin email |
...
```bash
cargo build --release     # produces target/release/rustcane
...
|---|---|---|---|
| `service.greet(name)` | `rustcane(action="greet", name="...")` | `rustcane greet [--name N]` | `name` optional in both |
| `service.echo(message)` | `rustcane(action="echo", message="...")` | `rustcane echo --message <msg>` | `message` required in both |
| `service.status()` | `rustcane(action="status")` | `rustcane status` | |
| _(MCP client interaction)_ | `rustcane(action="elicit_name")` | _(MCP-only — no CLI equivalent)_ | Requires elicitation-capable client |
| _(MCP elicitation wizard)_ | `rustcane(action="scaffold_intent")` | _(MCP-only — no CLI equivalent)_ | Combines elicitation + skill handoff; no one-shot CLI equivalent |
| _(built-in)_ | `rustcane(action="help")` | `rustcane --help` | MCP returns structured JSON; CLI prints usage |
...
- **`help` action is public** — `required_scope_for("help")` returns `None`. All other actions require at least `rustcane:read`.
- **Default port is 40060** — set in `default_mcp_port()` in `config.rs`. Override with `RUSTCANE_MCP_PORT`.
